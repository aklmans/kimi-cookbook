...
- PM2 使用单实例 cluster reload。新进程就绪后旧进程才收到退出信号，
  `kill_timeout=30s` 给 Next.js 足够时间排空请求。
...
```bash
ssh deploy@服务器IP 'node --version && pm2 --version && rsync --version'
```
...
3. `npx tsc --noEmit`
4. `npm run build`
5. 增量上传 standalone release
6. 从 Repository Secrets 原子同步 `shared/.env.production`
7. 原子切换 `current`，PM2 reload
8. 服务器本机校验 `/api/health` 的 git SHA
9. 从 GitHub Runner 通过公网域名再校验一次 DNS / TLS / Nginx
...
目标 SHA 必须已经存在于 `releases/`。脚本仍会做 PM2 reload、版本健康检查，
如果目标版本起不来，会恢复到回滚前的版本。

## 运维边界

- 同一台机器只运行一个 Next 实例，不需要 Redis cache handler。若以后扩成多台
  ECS，必须再解决共享缓存、tag invalidation 和 Server Actions encryption key。
