...
import type { BookAbout } from "@/lib/types";

/*「关于本书」for the Mini Program — single source for the about-page
   payload served by /api/mp/v1/book. Copy mirrors the web /about page;
   editing here ships to the MP within its 1h SWR cache, no MP release
   needed. All strings are plain text (no HTML / Markdown / newlines);
   section titles carry no trailing period (the MP adds the stop-dot). */

/** Feeds /api/mp/v1/version so about-only edits invalidate MP caches. */
export const ABOUT_UPDATED = "2026-07-19";

const site = (path: string) => absoluteUrl(path);
const siteLabel = (path: string) =>
...
export const about: BookAbout = {
  ...
  sections: [
    ...
    },
    {
      ...
      ways: [
        ...
        },
        {
          ...
          action: {
            kind: "copy",
            label: siteLabel("/books/kimi"),
            value: site("/books/kimi"),
            toast: "链接已复制",
          },
        },
        {
    ...
    },
    {
      ...
      license:
        "本书内容遵循 CC BY-NC-SA 4.0: 允许免费阅读、AI 摘读、引用与转发, 须保留作者署名 (Zhaphar) 与原文链接; 衍生作品须署名并同协议共享; 禁止商业再发布。",
